#!/usr/bin/env python3
"""Scrape ExamCompass Tech+ FC0-U71 quiz answers into Flash-Cards deck data.
...
TEST_NAME = "CompTIA Tech+ FC0-U71"
...
CONTENT_PATH = ROOT / "content" / "tests" / "tech-plus-fc0-u71.md"
CACHE_PATH = Path("/tmp/examcompass-tech-plus-fc0-u71-cache.json")
...
def classify_link(title: str, url: str) -> tuple[str, tuple[int, int, str], str, str] | None:
    ...
    lower_url = url.lower()
    if not title or "fc0-u71" not in lower_url:
        return None

    practice = re.search(r"Tech\+ Practice Test (\d+)", title, flags=re.I)
    if practice:
    ...
    if "quiz" in title.lower() and "certification-exam" in lower_url:
...
def extract_quiz_links() -> list[QuizLink]:
    ...
    if len(result) < 45:
        raise RuntimeError(f"Expected many Tech+ quiz links, found only {len(result)}")
    return result
...
def build_deck(links: Iterable[QuizLink], cache: dict[str, list[dict[str, object]]]) -> dict[str, list[dict[str, object]]]:
    ...
    for link in links:
        ...
        # with the numbered practice exams, but the requested deck should contain
        # all questions from all quizzes, not a globally deduplicated study set.
...
def next_test_index(menu: list[dict[str, object]]) -> int:
    return len(menu)


def update_menu(links: list[QuizLink]) -> None:
    menu = json.loads(MENU_PATH.read_text())
    menu = [item for item in menu if item.get("name") != TEST_NAME]
    ...
    menu.append(
        {
            ...
            "assetPath": "Tech+/",
            "chapter": [
                chapter_for_group("ExamCompass FC0-U71 Practice Exams", "#2B8CBE", by_group["practice"]),
                chapter_for_group("ExamCompass FC0-U71 Acronym Quizzes", "#8E44AD", by_group["acronyms"]),
                chapter_for_group("ExamCompass FC0-U71 Topic Quizzes", "#1F7A8C", by_group["topics"]),
            ],
        }
    )
...
def write_content_route() -> None:
    menu = json.loads(MENU_PATH.read_text())
    test_index = next(index for index, item in enumerate(menu) if item.get("name") == TEST_NAME)
    content = f"""---
title: \"{TEST_NAME}\"
testName: \"{TEST_NAME}\"
...
def main() -> None:
    ...
    if total_cards < 800:
        raise RuntimeError(f"Expected at least 800 cards, got {total_cards}")
